        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Navegacion principal">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`px-4 py-2 text-sm font-semibold tracking-wider transition-colors ${pathname === link.href
                  ? "text-primary"
          ...
          <nav className="flex flex-col px-4 py-4" aria-label="Navegacion movil">
            {navLinks.map((link) => (
              <Link
                ...
                className={`py-3 text-sm font-semibold tracking-wider border-b border-border ${pathname === link.href
                    ? "text-primary"
                    : "text-muted-foreground hover:text-foreground"
                  }`}
              >
